<template>
  <section id="user-interactivity">
    <article id="optionButtons">
      ...
      <button @click="displayShoppingList">
        Show this week's shopping list
      </button>
    </article>
    <p v-show="showRecipeSavedMsg">Recipe saved!</p>
    <form id="recipe-form" v-show="showForm" @submit.prevent="addRecipe">
      <p>
        <label for="title">Enter a recipe name:</label>
      </p>
      <p><input id="title" name="title" required v-model="title" /></p>
      <p><label for="ingredients">Enter an ingredient:</label></p>
      <p>
        <input id="ingredients" name="ingredients" v-model="ingredientName" />
      </p>
      ...
      <p>
        <label for="ingredient-weight"
          >Or, enter weight, measurement, etc:</label
        >
      </p>
      <p>
        <input
          ...
          v-model="ingredientWeight"
        />
      </p>
      <p v-if="ingredientQty && ingredientWeight">
        Please enter either quantity or free text!
      </p>
      <p v-if="showIngredientError">Ingredient name required!</p>
      <p>
        <button
          @click.prevent="addIngredient"
          :class="{ dualOptions: ingredientQty && ingredientWeight }"
        >
          Add ingredient
        </button>
      </p>
      ...
      <p>
        <select id="weekday" name="weekday" v-model="weekday">
          <option></option>
          <option v-for="weekday in weekdays" :key="weekday">
            {{ weekday }}
          </option>
        </select>
      </p>
      <p>
        <input type="submit" id="submit" name="submit" value="Save recipe" />
      </p>
      <p>
...
export default {
  ...
  methods: {
    ...mapActions(["saveRecipe", "saveRecipeActions", "saveRecipeUpdate"]),
    ...mapMutations([
      "hideAll",
      "showRecipe",
      "showRecipes",
    ...
    addIngredient() {
      if (
        !(this.ingredientWeight && this.ingredientQty) &&
        this.ingredientName
      ) {
        let ingredient = {
