<template>
  <div id="new-weekday">
    ...
    <select
      id="new-weekday-select"
      name="new-weekday-select"
      v-model="assignedWeekday"
    >
      <option v-if="currentRecipe.weekday">-- none --</option>
      <option v-for="(weekday, index) in weekdays" :key="index">
        {{ weekday }}
      </option>
    </select>
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["assignWeekday"]),
  },
  computed: {
    ...mapState(["weekdays", "currentRecipe"]),
    assignedWeekday: {
      get() {
